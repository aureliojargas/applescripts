...
--set YahooFields to {"Nome", "&nbsp;", "Sobrenome", "Apelido", "E-mail", "Categoria", "Listas de distribuição", "ID Yahoo!", "Informações pessoais", "Trabalho", "Pager", "Fax", "Celular", "Outro", "Fone Yahoo!", "Principal", "E-mail alternativo 1", "E-mail alternativo 2", "Web site pessoal", "Website da empresa", "Cargo", "Empresa", "Endereço", "Cidade", "Estado", "CEP", "País", "Endereço residencial", "Cidade", "Estado", "CEP", "País", "Data de nascimento", "Aniversário", "Complemento 1", "Complemento 2", "Complemento 3", "Complemento 4", "Comentários"}
...
on AvailableFilePath(FilePath, DefaultFileName)
	...
	tell application "Finder"
		...
		if folder FilePath exists then ¬
...
if not Vanilla then
	display dialog (ScriptName & return & return & ¬
		"I will scan your Address Book and dump all information to the Yahoo CSV format. " & ¬
	...
	if MaxContacts < TotalContacts then
		display dialog ScriptName & return & return & ¬
			"Warning: You have " & TotalContacts & " exportable contacts, " & ¬
			"but the current maximum is set to " & MaxContacts & "." & return & return & ¬
...
if not SaveOk then
	set FinalMessage to ScriptName & return & return & ¬
		"ERROR when trying to save the results to the following file:" & ¬
		return & return & tab & tab & (CsvFile as text)
	
else
	set FinalMessage to ScriptName & return & return & ¬
		(ContactCount as text) & ¬
		" contacts were exported from your Address Book to the following file:" & ¬
		return & return & tab & tab & (CsvFile as text)
